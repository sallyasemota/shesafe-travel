...
export function generateEmergencyPDF(trip: Trip): void {
  ...
  // Action statement (large, red, prominent)
  setRed(doc)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(13)
  const actionLines = doc.splitTextToSize(
    'Provide this document to local police and the embassy immediately. Also call the emergency contacts listed below.',
    USABLE_WIDTH,
  ) as string[]
  doc.text(actionLines, MARGIN, cursor.y)
  cursor.y += actionLines.length * 5 + 6
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFontSize(9)
    doc.text(
      'Generated by SheSafe Travel — for emergency use only',
      MARGIN,
      PAGE_HEIGHT - 14,
    )
    doc.text(
      `Issued ${new Date().toLocaleDateString()} for ${trip.traveler_name}`,
      MARGIN,
      PAGE_HEIGHT - 9,
    )
    doc.text(
